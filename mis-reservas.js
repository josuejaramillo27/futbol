...
document.getElementById('btn-login')?.addEventListener('click', async () => {
    try {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="ph-bold ph-spinner-gap"></i> Conectando...';
        await signInWithPopup(auth, new GoogleAuthProvider());
    } catch (error) {
        console.error("Error login:", error);
        alert("No pudimos conectar con Google. Revisa tu conexión.");
...
onAuthStateChanged(auth, user => {
    if (user) {
        ...
        contenedorReservas.style.display = 'grid';
        cargarMisReservas();
    } else {
...
async function cargarMisReservas() {
    ...
    contenedorReservas.innerHTML = '<div style="text-align:center; padding:40px; grid-column:1/-1;"><i class="ph-bold ph-spinner-gap ph-spin" style="font-size:32px;"></i><p>Buscando tus partidos...</p></div>';
    ...
    try {
        ...
        if (reservas.length === 0) {
            contenedorReservas.innerHTML = `
                <div class="empty-state" style="text-align:center; padding: 50px 20px; grid-column:1/-1; background: rgba(255,255,255,0.02); border-radius:15px;">
                    ...
                    <a href="index.html" class="btn hero-primary" style="display:inline-block; width:auto;">Buscar Canchas</a>
                </div>
            `;
            return;
        }
        ...
        contenedorReservas.innerHTML = '<p style="color:var(--danger); text-align:center; grid-column:1/-1;">Hubo un error al cargar tus reservas.</p>';
    }
}
...
function renderReservas(reservas) {
    contenedorReservas.innerHTML = reservas.map(r => {
        ...
        return `
        <article class="reserva-card" style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05); border-radius:15px; padding:20px; display:flex; justify-content:space-between; flex-wrap:wrap; gap:15px;">
            <div>
...
async function intentarCancelar(reservaId, fechaStr, horaStr) {
    ...
    const diffHours = (resDate - new Date()) / (1000 * 60 * 60);

    if (diffHours < 2 && diffHours > 0) {
        alert("Política de Cancelación:\n\nNo puedes cancelar desde la aplicación si faltan menos de 2 horas. Comunícate directamente con la cancha.");
        return;
    }
    if (diffHours <= 0) return alert("Esta reserva ya pasó.");
    if (!confirm("¿Estás seguro de cancelar esta reserva?")) return;

    try {
        await updateDoc(doc(db, 'reservas', reservaId), {
            estado: 'cancelled', canceladoPor: 'player', canceladoEn: serverTimestamp(), updatedAt: serverTimestamp()
        });
